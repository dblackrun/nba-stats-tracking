...
def test_get_tracking_response_json_for_stat_measure():
    ...
    response_json = tracking.get_tracking_response_json_for_stat_measure(measure_type, season, season_type, entity_type)
...
def test_team_aggregate_full_season_tracking_stats_for_seasons():
    ...
    stats, league_totals = tracking.aggregate_full_season_tracking_stats_for_seasons(measure_type, seasons, season_types, entity_type)
    assert len(stats) == 30
    for stat in stats:
    ...
    assert league_totals == {'MIN': 956470.0, 'CATCH_SHOOT_FGM': 37981, 'CATCH_SHOOT_FGA': 101524, 'CATCH_SHOOT_PTS': 109049, 'CATCH_SHOOT_FG3M': 33087, 'CATCH_SHOOT_FG3A': 90075}
...
def test_generate_tracking_game_logs():
    ...
    game_logs = tracking.generate_tracking_game_logs(measure_type, entity_type, date, date)
